...
class TB(nn.Module):
    def __init__(self, weightPTH):
        ...
        checkpoint = torch.load(weightPTH)
...
class FCBFormer(nn.Module):
    def __init__(self, size=352, weightPth="", n_classes=1):

        super().__init__()

        self.TB = TB(weightPth)

        self.FCB = FCB(in_resolution=size)
        self.PH = nn.Sequential(
            RB(64 + 32, 64), RB(64, 64), nn.Conv2d(64, n_classes, kernel_size=1)
        )
